...
void heap_pop(Heap *pq) {
  ...
  while ((2 * padre) + 1 < pq->size) // buscar padres
  {
    int hijo = 2 * padre + 1;
    if (hijo + 1 < pq->size &&
        pq->heapArray[hijo + 1].priority >
            pq->heapArray[hijo].priority) // hijo de mayor prioridad
    {
      hijo++;
    }

    if (pq->heapArray[padre].priority <
        pq->heapArray[hijo]
            .priority) // si el hijo tiene mayor prioridad que el padre
    {
